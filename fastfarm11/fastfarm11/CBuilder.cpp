...
bool CBuilder::OnInit(){
	...
	FILE* fp = fopen("../maps/index.level","r");
	  if(fp==NULL) {printf("error reading index"); Running=2;  return false;}
	  fscanf(fp,"%d",&level);
	  fclose(fp);
	  ++level;
...
void CBuilder::OnRender(){
	...
	for (int i = FARM_i; i < FARM_HEIGHT+FARM_i; i++)
		for (int j = 0; j < FARM_WIDTH+FARM_j; j++)
		{
			...
			{plant.X = j*TILE_SIZE+TILE_SIZE/2 - plant.getCenter().first;  plant.Y = i*TILE_SIZE+TILE_SIZE/2 - plant.getCenter().second;
				plant.OnRender(ren);
	        }
...
void CBuilder::OnLButtonDown(int mX, int mY){
	...
	{	if(farm[i-FARM_i][j-FARM_j]==1 || farm[i-FARM_i][j-FARM_j]==2) farm[i-FARM_i][j-FARM_j]=3;
		else
		if(farm[i-FARM_i][j-FARM_j]==3) farm[i-FARM_i][j-FARM_j]=0;
		else
		if(surround(i,j,farm)<4 && check8(i,j,farm,-1)) 
	...
	sprintf(str,"../maps/%d.level",level);
	  FILE* fp = fopen(str,"w");
	  if(fp==NULL) {printf("error saving level"); Running=2;  return;}
	  for (int i = FARM_i; i < FARM_HEIGHT+FARM_i; i++)
	  {for (int j = FARM_j; j < FARM_WIDTH+FARM_j; j++)
	  ...
	  if(fp==NULL) {printf("error writing index"); Running=2;  return ;}
	  fprintf(fp,"%d",level);
	  fclose(fp);
	  ++level;
