...
{int reprod=0;
	...
	for(int i = FARM_i; i<FARM_HEIGHT+FARM_i; ++i)
	{
		...
		{
			int count = 0;
			if(farm[i-FARM_i][j-FARM_j] == 0 || farm[i-FARM_i][j-FARM_j] == 3) continue;
			...
			if(count == 4 && farm[i-FARM_i][j-FARM_j]==1) {++reprod;  farm[i-FARM_i][j-FARM_j]=2;}
...
void CPlant::OnLoop()
{
	...
	if(farm[i-FARM_i][j-FARM_j]==0) {alive = false;  //printf("%d %d\n",i,j);
	}
	if(farm[i-FARM_i][j-FARM_j]== 2) AnimState =1;
