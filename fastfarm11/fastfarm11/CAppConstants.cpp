...
{int count=0;
if(i<FARM_i || i>=FARM_HEIGHT+FARM_i || j<FARM_j || j>=FARM_WIDTH+FARM_j) return -1;
...
{  
	for(int m=i-1; m<=i+1; ++m)
		for(int n=j-1; n<=j+1; ++n)
		{  if(m==n && choice==-1) continue; 
			if(surround(m,n,farm)>=4 && (farm[m-FARM_i][n-FARM_j]==1||farm[m-FARM_i][n-FARM_j]==2)) return false;}
	return true;
}
