...
{
	if(!tim.Started()) tim.Start();
	if(no_init_pop1==0 && tim.Ticks()/1000>initPeriod) 
	{if(ratio1_2>1) pl1.AnimState=2;
	else if(ratio1_2<1) pl1.AnimState=1;
	else pl1.AnimState=0;
	}
	if(no_init_pop2==0 && tim.Ticks()/1000>initPeriod) 
	{if(ratio1_2>1) pl2.AnimState=1;
	else if(ratio1_2<1) pl2.AnimState=2;
	else pl2.AnimState=0;
	}
	pl1.OnLoop();	pl2.OnLoop();
}
...
void CPlayersM::OnEvent(SDL_Event* Event){
	...
	switch(Event->type)
	{
	case SDL_FINGERDOWN: {  int tileid = (int)(Event->tfinger.x*SCREEN_WIDTH)/TILE_SIZE + ((int)(Event->tfinger.y*SCREEN_HEIGHT)/TILE_SIZE)*MAP_WIDTH;
	...
	case SDL_FINGERMOTION:	{
								if(Event->tfinger.fingerId==recEvent1.fingerId && (recEvent1.type==SDL_FINGERDOWN || recEvent1.type==SDL_FINGERMOTION))
								{
									...
									if(x!=pl2.X || y!=pl2.Y) {pl1.X = x;  pl1.Y = y;}
									recEvent1.fingerId = Event->tfinger.fingerId;
									recEvent1.type = Event->type;
								}
								else if(Event->tfinger.fingerId==recEvent2.fingerId && (recEvent2.type==SDL_FINGERDOWN || recEvent2.type==SDL_FINGERMOTION))
								{
									...
									int y = ((int)(Event->tfinger.y*SCREEN_HEIGHT)/TILE_SIZE)*TILE_SIZE;
									if(x/TILE_SIZE>=FARM_j && x/TILE_SIZE<FARM_j+FARM_WIDTH && y/TILE_SIZE>=FARM_i && y/TILE_SIZE<FARM_HEIGHT+FARM_i)
									if(x!=pl1.X || y!=pl1.Y) {pl2.X = x;  pl2.Y = y;}
									recEvent2.fingerId = Event->tfinger.fingerId;
									recEvent2.type = Event->type;
								}
								break;
	...
	case SDL_FINGERUP:	{  printf("touchup x %f y %f\n",Event->tfinger.x*CSurface::scaleX*SCREEN_WIDTH,Event->tfinger.y*CSurface::scaleY*SCREEN_HEIGHT);
							if(Event->tfinger.fingerId==recEvent1.fingerId && (recEvent1.type==SDL_FINGERDOWN || recEvent1.type==SDL_FINGERMOTION))
								{
									if(recEvent1.type==SDL_FINGERDOWN)
									{
										//take action like planting
										if(CPlant::farm[(int)pl1.Y/TILE_SIZE-FARM_i][(int)pl1.X/TILE_SIZE-FARM_j]) break;
										if(no_init_pop1>0)
										{
											--no_init_pop1; goto lowf1;
										}
										else
										if(tim.Ticks()/1000<initPeriod){ goto normf1;}
										else
										if(ratio1_2>1) {
								...
								else if(Event->tfinger.fingerId==recEvent2.fingerId && (recEvent2.type==SDL_FINGERDOWN || recEvent2.type==SDL_FINGERMOTION))
								{
									...
									{
										//take action like planting
										if(CPlant::farm[(int)pl2.Y/TILE_SIZE-FARM_i][(int)pl2.X/TILE_SIZE-FARM_j]) break;
										if(no_init_pop2>0)
										{
											--no_init_pop2; goto lowf2;
										}
										else
										if(tim.Ticks()/1000<initPeriod){ goto normf2;}
										else
										if(ratio1_2<1) {
											CEntity* plantC = new CPlantM();
											plantC->OnLoad("../tilesets/farmsheet2.png",82,82,8,ren);
	...
	case SDL_KEYDOWN:	{ if(Event->key.repeat) break;
						switch(Event->key.keysym.sym)
						{
							...
							case SDLK_LCTRL :{
											...
											if(no_init_pop1>0)
											{
												--no_init_pop1; goto lowk1;
											}
											else
											if(tim.Ticks()/1000<initPeriod){ goto normk1;}
											else
											if(ratio1_2>1) {
												CEntity* plantC = new CPlantM();
												plantC->OnLoad("../tilesets/farmsheet2.png",82,82,8,ren);
							...
							case SDLK_RCTRL :{
										...
										if(no_init_pop2>0)
										{
											--no_init_pop2; goto lowk2;
										}
										else
										if(tim.Ticks()/1000<initPeriod){ goto normk2;}
										else
										if(ratio1_2<1) {
											CEntity* plantC = new CPlantM();
											plantC->OnLoad("../tilesets/farmsheet2.png",82,82,8,ren);
   ...
   CArea::AreaControl.OnLoad("../maps/1.area", ren);
	...
	while(SDL_PollEvent(&Event)) {
		if(Event.type==SDL_KEYDOWN)
		 {	switch(Event.key.keysym.sym)
			{
			case SDLK_LEFT: {gamePeriod -=1; if(gamePeriod<minPeriod) gamePeriod=maxPeriod;
							break;}
			case SDLK_RIGHT: {gamePeriod+=1; if(gamePeriod>maxPeriod) gamePeriod=minPeriod;
							break;}
			case SDLK_RETURN: { goto end;							
							break;}
		...
		else if(Event.type==SDL_MOUSEBUTTONDOWN)
		 {
			 if(Event.button.button==SDL_BUTTON_LEFT)
			 {
				 ...
				 if(i==MAP_HEIGHT/2 && j==MAP_WIDTH/2-3)
					{gamePeriod -=1; if(gamePeriod<minPeriod) gamePeriod=maxPeriod;}
				 else if(i==MAP_HEIGHT/2 && j==MAP_WIDTH/2+2)
					 {gamePeriod+=1; if(gamePeriod>maxPeriod) gamePeriod=minPeriod;}
